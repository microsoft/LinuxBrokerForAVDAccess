-- Redefines dbo.TriggerScalingLogic for scaling schedules (2.5).
--
...
-- * Scaling down to get under MaxVMs never takes serviceable hosts below MinVMs. Draining and
--   maintenance hosts count toward MaxVMs, so without this a maintenance run that starts hosts
--   could make the scaler stop the ready ones.
-- * A run waits up to five seconds for the scaling lock, which maintenance admission holds
--   briefly, rather than skipping.
-- * Each power-on stamps StartRequestedAt for the start-to-ready measure, a power-off clears
--   it, and the activity log records the phase, its minimum and maximum, and the serviceable
--   and draining counts.
--
