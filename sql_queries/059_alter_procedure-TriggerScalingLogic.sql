...
BEGIN
    ...
    DECLARE @PoweredOn INT = 0, @Serviceable INT = 0, @InUse INT = 0, @Utilization DECIMAL(7,2) = 0,
    ...
    SELECT @PoweredOn = SUM(CASE WHEN PowerState = 'On' THEN 1 ELSE 0 END),
           ...
           @Serviceable = SUM(CASE WHEN PowerState = 'On' AND VmStatus <> 'Maintenance'
                               ...
                               THEN 1 ELSE 0 END),
           @InUse = SUM(CASE WHEN PowerState = 'On' AND (VmStatus IN ('CheckedOut', 'Released') OR CleanupPending = 1) THEN 1 ELSE 0 END)
    ...
    BEGIN
        ;WITH Candidates AS (
            ...
            WHERE PowerState = 'On' AND NetworkStatus = 'Reachable' AND VmStatus = 'Available'
              AND Username IS NULL AND LeaseId IS NULL AND CleanupPending = 0
    ...
    SET @Notes = CONCAT(N'Rule Min=', @MinVMs, N', Max=', @MaxVMs, N', UpRatio=', @ScaleUpRatio, N', DownRatio=', @ScaleDownRatio,
        ...
        N', serviceable=', @Serviceable, N', inUse=', @InUse, N', utilization=', @Utilization, N'%. ', @Reason, @Corrections);
