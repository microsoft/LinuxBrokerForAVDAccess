...
# Screen lock policy is now generated by apply-host-settings.sh from the fleet-wide settings
# profile, on every supported distribution rather than only this one. Seeding the defaults
# here leaves the host in the same state it had before, and the release agent converges it
# to the configured profile on its next run.
