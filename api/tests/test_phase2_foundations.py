...
def health_row(hostname, **values):
    row = {
        ...
        "HeartbeatAgeSeconds": 30, "AgentVersion": "1.0.0",
        "ScriptVersionsJson": '{"release-session.sh": "1.0.0", "create-user.sh": "1.0.0"}',
...
def test_fleet_health_flags_what_an_operator_must_act_on(client, fake_db):
    fake_db.fetchall_rows["GetHostHealth"] = [
        ...
        health_row("half", ScriptVersionsJson='{"release-session.sh": "1.0.0", "manage-lease.sh": null}'),
    ...
    assert healthy["ScriptVersions"]["create-user.sh"] == "1.0.0"
    assert body["ExpectedAgentVersion"] == "1.0.0" and body["StaleAfterSeconds"] == 180
